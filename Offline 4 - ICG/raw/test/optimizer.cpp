...
const string registers[] = {"AX", "BX", "CX", "DX", "SI", "DI", "BP", "SP", "AL", "BL", "CL", "DL", "AH", "BH", "CH", "DH"};

bool is_register(const std::string& str) {
    return find(begin(registers), end(registers), str) != end(registers);
}
    ...
    for(int i=0; i<n; i++){
        ...
        if(i<n-1){
            ...
            if(s1.op == "MOV" && s2.op == "MOV"){
                // Case 1:
                if( is_register(s1.args[0]) && !is_register(s1.args[1])){
                ...
                // Case 2:
                else if( !is_register(s1.args[0]) && is_register(s1.args[1])){
                    if(s1.args[0] == s2.args[1] && s1.args[1] == s2.args[0]){
                        marked[i] = 1;
                        //marked[i+1] = 1;
                    }
