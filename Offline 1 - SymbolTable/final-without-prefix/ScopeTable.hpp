...
public:
	...
	SymbolInfo* lookup(string name, bool verbose = false)
	{
		...
		while(curr != NULL)
		{
			...
			{
				if(verbose) cout<<"'"<<name<<"' found at position <"<<index+1<<", "<<index_in_chain+1<<"> of ScopeTable# "<<id<<endl;
				return curr;
			}
			curr = curr->next;
			index_in_chain++;
	...
	bool insert(string name, string type, bool verbose = false)
	{
		...
		if(existing_entry == NULL)  // does not already exist
		{
			...
			if(verbose) cout<<"Inserted  at position <"<<index+1<<", "<<index_in_chain+1<<"> of ScopeTable# "<<id<<endl;
			return true;
		}
	...
	bool remove(string name, bool verbose = false)
	{
		...
		if(existing_entry != NULL)
		{
			...
			temp->next = NULL;
			delete temp;
			if(verbose) cout<<"Deleted '"<<name<<"' from position <"<<index+1<<", "<<index_in_chain+1<<"> of ScopeTable# "<<id<<endl;
			return true;
		}
		else
		{
